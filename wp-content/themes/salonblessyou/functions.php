...
add_image_size('thumb_size', 124, 124,true);
...
function breadcrumb(){
  ...
  $str.= '<ol itemprop="Breadcrumb" itemscope itemtype="http://data-vocabulary.org/BreadcrumbList">';
  $str.= '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a itemprop="item" href="'.home_url('/').'" class="home"><span itemprop="name">TOP</span></a><meta itemprop="position" content="1"></li>';
  ...
  elseif(is_category()) {
    ...
    if($cat->parent != 0){
      $ancestors = array_reverse(get_ancestors($cat->cat_ID, 'category'));
      foreach($ancestors as $ancestor){
        $str.= '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a itemprop="item" href="'. get_category_link($ancestor) .'"><span itemprop="name">'.get_cat_name($ancescattor).'</span></a><meta itemprop="position" content="'.$pNum.'"></li>';
      }
    }
